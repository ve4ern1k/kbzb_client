<template>
    <div style="display: flex; flex-direction: column; margin: 120px 0px">
        <p style="margin-bottom: 32px; font-size: 32px; font-family: 'Source Sans Pro'; font-weight: 600;">Полистиролбетон</p>

        <div style="width: 100%;">
            <div style="float: left; margin: 0px 32px 0px 0px">
                <img src="/product7.jpg" height="300px" style="border-radius: 4px;" />
            ...
            <div style="display: flex; flex-direction: column; align-items: center;">
                <p style="font-size: 18px; font-family: 'Source Sans Pro'; font-weight: 300; text-indent: 1.5em;">Полистиролбетон (ПСБ) – особо легкий бетон с пористой структурой, производимый на базе цементного вяжущего (обычно портландцемент), с пористым гранулированным заполнителем (вспененный полистирол).</p>
                <p style="font-size: 18px; font-family: 'Source Sans Pro'; font-weight: 300; text-indent: 1.5em;">Также в раствор добавляются воздухововлекающие вещества и различные модифицирующие добавки, улучшающие свойства как смеси, в процессе применения, так и готового бетона в процессе эксплуатации. Воздухововлекающие добавки, такие как СДО (смола древесно омыленная), необходимы для придания цементному камню пористой структуры не только за счет полистирольных шариков, но и за счет образования замкнутых воздушных ячеек.</p>
            </div>
    ...
    async created() {
        ...
        await fetch(url1, query1)
            .then(async (response) => {
                if (response.status !== 200) {
                    ...
                    this.concreteTypes = jsonBody.data.products;
        ...
        this.$store.state.basket.map(elStore => {
            this.concreteTypes.map(elHere => {
                if ((elStore.id === elHere.id) && (elStore.category === 'Полистиролбетон')) {
